 ...
 * [
 * _module,
 * (_module getVariable QCLASS(AI_Zone_faction)),
 * (_module getVariable QCLASS(AI_Zone_Class)),
 * (_module getVariable QCLASS(AI_Zone_GroupMAX)),
 * (_module getVariable QCLASS(AI_Zone_WeapPrimaryLoot)),
 * (_module getVariable QCLASS(AI_Zone_WeapSecondaryLoot)),
 * (_module getVariable QCLASS(AI_Zone_WeaplauncherLoot)),
 * (_module getVariable QCLASS(AI_Zone_Ammo)),
 * (_module getVariable QCLASS(AI_Zone_ItemLoot)),
 * (_module getVariable QCLASS(AI_Zone_NVGLoot)),
 * (_module getVariable QCLASS(AI_Zone_FacewearLoot)),
 * (_module getVariable QCLASS(AI_Zone_HeadgearLoot)),
 * (_module getVariable QCLASS(ai_zone_UniformLoot)),
 * (_module getVariable QCLASS(AI_Zone_VestLoot)),
 * (_module getVariable QCLASS(AI_Zone_BackpackLoot)),
 * (_module getVariable QCLASS(AI_Zone_Accuracy)),
 * (_module getVariable QCLASS(AI_Zone_Shake)),
 * (_module getVariable QCLASS(AI_Zone_Speed)),
 * (_module getVariable QCLASS(AI_Zone_SpawnChance)),
 * (_module getVariable QCLASS(AI_Zone_ModuleSpawnDistanceMIN)),
 * (_module getVariable QCLASS(AI_Zone_ModuleSpawnDistanceMAX))
 * ] call misery_ambient_ai_zone_fnc_moduleCheck;
 *
 * Public: No
*/

private _modules = allMissionObjects QCLASS(Ambient_AI_Zone);
...

    {
        private _player = _x;
        {
            ...
            if (_distance > Misery_AmbAI_SpawnCheckDistance && _distance < _maxSpawnDistance) then {
                ...
                private _AI_ZoneSpawned = _module getVariable [QCLASS(AI_Zone_Spawned), false];

                if (!(_AI_ZoneSpawned) && isServer) then {

                _module setVariable [QCLASS(AI_Zone_Spawned), true, true];
