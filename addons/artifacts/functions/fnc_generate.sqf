...
if (isServer) then {
...

    {
        ...
        if (_x call EFUNC(artifacts,safe)) then {
            [QUOTE(COMPONENT_BEAUTIFIED), "Artifact spawned under a roof or inside of a building. Deleting."] call EFUNC(common,debugMessage);
            deleteVehicle _x;
        } else {
    ...
    while {true} do {
        ...
        if (count((weaponCargo _groundStash)+(itemCargo _groundStash)+(magazineCargo _groundStash)+(backpackCargo _groundStash)+(weaponsItemsCargo _groundStash)) < 1) exitWith {
            [QUOTE(COMPONENT_BEAUTIFIED), "Loot pile no longer has an artifact, deleting."] call EFUNC(common,debugMessage);
