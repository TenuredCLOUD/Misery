 ...
 *
 * Example:
 * (([ACE_player] call misery_common_fnc_isInside) select 0)
 * (([ACE_player] call misery_common_fnc_isInside) select 1)
*/

private ["_playerPos","_isUnderRoof","_wallCount","_playerDir","_maxDistance","_endPos","_introof","_intwalls","_wall","_roof"];

_playerPos = getPosWorld ACE_player;
_isUnderRoof = false;
_wallCount = 0;
...
_introof = lineIntersectsSurfaces [_playerPos,_endPos,ACE_player,objNull,true,1,"GEOM","NONE"];
...
for "_i" from 0 to 3 do {
    ...
    _endPos = _playerPos vectorAdd (_playerDir vectorMultiply _maxDistance);
    _intwalls = lineIntersectsSurfaces [_playerPos,_endPos,ACE_player,objNull,true,1,"GEOM","NONE"];
