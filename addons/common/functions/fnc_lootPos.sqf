...
if((count _posIn)isEqualTo0)exitWith{[]};
_safe=true;
_pos=_posIn;
if!(_pos call FUNC(IsInsideloot))then{_safe=false};
...
if((count _pos)isEqualTo0)then{_pos=[_posIn select 0,_posIn select 1,0]};//1
_pos
