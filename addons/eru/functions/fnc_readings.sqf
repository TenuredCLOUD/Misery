    ...
    [{
        ...
        switch (true) do {
            case (!isNil QGVAR(temperatureEnvironment)): {
                _textControlReadings ctrlSetText format ["%3 %1°C %4 %2°C", parseNumber (((call EFUNC(temperature,environment)) select 0) toFixed 1), parseNumber (((call EFUNC(temperature,environment)) select 1) toFixed 1), localize ECSTRING(common,AirLabel), localize ECSTRING(common,SeaLabel)];
            };
            case (!isNil QGVAR(temperatureBody)): {
                call EFUNC(common,getPlayerVariables) params ["", "", "", "_thermalIndex"];
                _textControlReadings ctrlSetText format ["%2 %1°C", [_thermalIndex, 1, 1, false] call CBA_fnc_formatNumber, localize ECSTRING(common,ExposureLabel)];
            };
            case (!isNil QGVAR(compass)): {
                ...
                _textControlReadings ctrlSetText format ["%3 %1 %2", _direction, _Bearing, localize ECSTRING(common,CompassLabel)];
            };
            default {
                _textControlReadings ctrlSetText localize ECSTRING(common,NoData);
            };
        };
