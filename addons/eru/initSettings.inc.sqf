...
[_category, QGVAR(power), localize ECSTRING(common,ERUActionToggleERU), {
    call FUNC(keybind);
}, {}, [DIK_U, [false, false, false]]] call CBA_fnc_addKeybind;

[_category, QGVAR(compass), localize ECSTRING(common,ERUCompassSetting), {
...
[_category, QGVAR(temperature_0), localize ECSTRING(common,ERUTempEnv), {
    if ([[QCLASS(eru_On)]] call EFUNC(common,hasItem)) then {
...
[_category, QGVAR(temperature_1), localize ECSTRING(common,ERUTempExp), {
    if ([[QCLASS(eru_On)]] call EFUNC(common,hasItem)) then {
        GVAR(temperatureEnvironment) = nil;
