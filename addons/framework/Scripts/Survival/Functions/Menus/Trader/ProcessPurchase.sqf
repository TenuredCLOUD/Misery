...
if (_playerFunds >= _price) then {
    if (_stock < 1) exitWith {ctrlSetText [1001, format ["There isn't enough stock to purchase %1...",_displayName]]; [] execVM '\z\misery\addons\framework\scripts\survival\functions\menus\Trader\ShopVALs.sqf';};
