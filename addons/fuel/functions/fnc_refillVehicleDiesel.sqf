...
if (_Found && ((_Array select 1) == 1)) exitwith {titleText [format["%1 Requires Petrol...", _Vehiclename], "PLAIN DOWN"];};
if (_Found && ((_Array select 1) == 2)) exitwith {titleText [format["%1 Requires Jetfuel...", _Vehiclename], "PLAIN DOWN"];};

if (((_Array select 1) == 0)) exitwith {
if (_Found) then {
    ...
    if ((player distance MiseryTarget_Veh) < _ArrayDistcheck) then {
            ...
            player addItem "Misery_EmptyDiesel"; //Add empty canister
