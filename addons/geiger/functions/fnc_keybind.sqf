...
if (_hasOff) then {
    _text = format ["<t font='PuristaMedium' size='0.7'>%1</t>", localize "STR_MISERY_TURNONGEIGERBOOTUP"];
    playSound QEGVAR(audio,sound_geigerOff);
    [[QCLASS(geiger_Off)], [QCLASS(geiger_On)]] call EFUNC(common,switchMagazine);
};

if (_hasOn) then {
    _text = format ["<t font='PuristaMedium' size='0.7'>%1</t>", localize "STR_MISERY_TURNONGEIGERTURNOFF"];
...
if !(_hasOn && _hasOff) then {
    _text = format ["<t font='PuristaMedium' size='0.7'>%1</t>", localize "STR_MISERY_TURNONGEIGERNOITEM"];
};
