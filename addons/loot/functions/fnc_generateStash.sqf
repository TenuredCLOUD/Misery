...
if (!isNil "grad_persistence_blacklist") then {
    if ((grad_persistence_blacklist find (toLower "GroundWeaponHolder_Scripted") == -1) && (grad_persistence_blacklist find (toUpper "GroundWeaponHolder_Scripted") == -1)) then {
        ...
        if (MiseryDebug) then {
            systemChat "[Misery Loot Framework] GRAD Persistence detected, Adding loot piles to blacklist for saving / reloading..."
        };
    };
};
...
if (isNull _vehicle) then {
    ...
    _pos=[_pos, _crateType]call EFUNC(common,LootPos);
    _groundStash=createVehicle[_crateType, _pos, [], 0, "can_Collide"];
    _groundStash setDir(random 360);
} else {
    _groundStash=_vehicle;
    _pos=getPos _groundStash
};
if ((count _pos)<2) exitWith {};
...
while { (count _players) > 0 } do {
    if (MiseryDebug) then {
        systemChat format["[Misery Loot Framework] Populating lootpile at %1", getPosATL _groundStash];
    };
    ...
    if ((random 100)<_chance) then {
        while { _count>0 } do {
            ...
            if (_index in[0, 1, 2, 4, 5, 6, 7, 9]) then {
                _groundStash addItemCargoGlobal[_item, 1]
            };
            if (_index==8) then {
                _groundStash addBackpackCargoGlobal[_item, 1]
            };
            if (_index>9) then {
                _groundStash addMagazineCargoGlobal[_item, 1]
            };
            if (_index==3) then {
...
if (MiseryLootDeleteDist == -1) then {
    systemChat format["[Misery Loot Framework] Skipping cleanup due to %1 Distance check value...", MiseryLootDeleteDist];
};

while { (count _players) > 0 && MiseryLootDeleteDist != -1 } do {
    if (MiseryDebug) then {
        systemChat format["[Misery Loot Framework] Checking Lootpile at %1 distance for deletion...", getPosATL _groundStash];
    };
        if (isNull _groundStash) exitWith {}; // in case it gets deleted by other means
    {
        _deleteFlag=true;
        _player=_x;
        if ((_groundStash distance _player) < MiseryLootDeleteDist) exitWith {
            _deleteFlag=false
        };
    } forEach _players;
    if (count((weaponCargo _groundStash)+(itemCargo _groundStash)+(magazineCargo _groundStash)+(backpackCargo _groundStash)+(weaponsItemsCargo _groundStash)) < 1) exitWith {
        if (MiseryDebug) then {
            systemChat format["[Misery Loot Framework] Loot pile generated with no items at %1 Deleting...", getPosATL _groundStash];
        };
        deleteVehicle _groundStash;
    };// Delete empty piles (sometimes generated with low chances - this fixes that)
    if (_deleteFlag) exitWith {
        if (MiseryDebug) then {
            systemChat format["[Misery Loot Framework] Loot pile exceeds %2m distance. Deleting lootpile at %1...", getPosATL _groundStash, MiseryLootDeleteDist];
        };
        deleteVehicle _groundstash;
...
if (count((weaponCargo _groundStash)+(itemCargo _groundStash)+(magazineCargo _groundStash)+(backpackCargo _groundStash)+(weaponsItemsCargo _groundStash)) < 1) then {
    deleteVehicle _groundStash;
    if (MiseryDebug) then {
        systemChat format["[Misery Loot Framework] Loot pile empty at %1 Deleting...", getPosATL _groundStash];
    };
