...
if (_missingBatteries <= 0) exitWith {
    ctrlSetText [1001, format [localize LSTRING(AllBatteriesInstalled)]];
};
...
if (_playerBatteryCount >= 1) then {
    ...
    [{
        ...
        [ACE_player, _batteryClass] call CBA_fnc_removeMagazine;
        ...
        ctrlSetText [1001, format [localize LSTRING(BatteryInstalledCount), [_batteryClass] call EFUNC(common,getItemData) select 0, _installedBatteries, _batteryCount]];
    ...
    ctrlSetText [1001, format [localize LSTRING(NeedItemToInstall), [_batteryType] call EFUNC(common,getItemData) select 0]];
};
