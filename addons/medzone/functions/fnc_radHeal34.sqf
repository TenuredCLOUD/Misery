...
_pricerads = parseNumber (((((player getVariable [QCLASS(radiation),0]) * 0.75) / 50) * Mis_Medpriceradheal) toFixed 1);
...
_priceradscalc = parseNumber (((player getVariable [QCLASS(radiation),0]) * 0.75) toFixed 1);
