...
//Debug markers:
if(MiseryDebug)then{_marker=createMarkerLocal[format["%1",_b],(getPos _b)];_marker setMarkerTypeLocal"mil_dot";_marker setMarkerColorLocal"ColorBlue"};
_bText=toLower(getText(configfile>>"CfgVehicles">>(typeOf _b)>>"editorSubcategory"));

// Military building
if((_bText find"military")>-1)exitWith{
	if(MiseryDebug)then{_marker setMarkerTypeLocal"mil_box";_marker setMarkerColorLocal"ColorGREEN"};
	...
	}else{if(MiseryDebug)then{_marker setMarkerColorLocal"ColorYellow"}};
};
