...
if (EGVAR(psychosis,enabled) && EGVAR(psychosis,sleepBlocked)) exitWith {
    [QEGVAR(common,tileText), localize ECSTRING(common,CantSleepFear)] call CBA_fnc_localEvent;
...
if (_selectedHour isEqualTo 0) exitWith {
    [QEGVAR(common,tileText), localize ECSTRING(common,SleepNoHourSelect)] call CBA_fnc_localEvent;
};

if (_energyDeficit < 0.15) exitWith {
    [QEGVAR(common,tileText), localize ECSTRING(common,SleepNotTired)] call CBA_fnc_localEvent;
};
...
[{
    [QEGVAR(common,tileText), format [localize ECSTRING(common,SleptForHowLong), _this]] call CBA_fnc_localEvent;
    player setFatigue 1;
