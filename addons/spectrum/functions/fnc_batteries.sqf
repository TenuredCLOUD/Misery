...
if (_light isNotEqualTo "") exitWith {
    [QEGVAR(common,tileText), localize LSTRING(RemoveLight)] call CBA_fnc_localEvent;
};

if  ([QCLASS(lithiumBattery)] call EFUNC(common,countItem) < 2) exitWith {
    [QEGVAR(common,tileText), localize LSTRING(NoLithium)] call CBA_fnc_localEvent;
};

if !([["hgun_esd_01_dummy_F"]] call EFUNC(common,hasItem)) exitWith {
    [QEGVAR(common,tileText), localize LSTRING(NoDevice)] call CBA_fnc_localEvent;
};
