...
private _damaged = ACE_player call ACEFUNC(medical,isInjured);
...
ACE_player setVariable [QGVAR(parasites), _finalParasites];

if (_parasites > 0) then {
    [3.33333e-4, "parasites"] call EFUNC(common,addStatusModifier);
};
if (_parasites < 0) then {
    ACE_player setVariable [QGVAR(parasites), MACRO_PLAYER_DEFAULTS_LOW];
};
...
ACE_player setVariable [QGVAR(infection), _finalInfection];

if (_infection > 0) then {
    if (_damaged || _hunger < 0.75 || _thirst < 0.75) then {
        [3.33333e-4, "infection"] call EFUNC(common,addStatusModifier);
    } else {
        [-1.66667e-4, "infection"] call EFUNC(common,addStatusModifier);
    };
};
if (_infection < 0) then {
    ACE_player setVariable [QGVAR(infection), MACRO_PLAYER_DEFAULTS_LOW];
};
