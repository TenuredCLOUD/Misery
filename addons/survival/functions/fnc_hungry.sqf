...
[{!(isNil {player getVariable QEGVAR(survival,hunger)}) && ((player getVariable [QEGVAR(survival,hunger), MACRO_PLAYER_HUNGER]) < 75)},
{

    [{
        params ["_args", "_handle"];

        if ((player getVariable [QEGVAR(survival,hunger), MACRO_PLAYER_HUNGER]) > 75 || (!alive player)) exitWith {
    ...
    if ((player getVariable [QEGVAR(survival,hunger), MACRO_PLAYER_HUNGER]) < 75) then {
