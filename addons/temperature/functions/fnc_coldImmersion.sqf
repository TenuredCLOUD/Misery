...
[{((player getVariable [QGVAR(exposure), MACRO_PLAYER_DEFAULTS_LOW]) < -0.1)},
{
    ...
    if (((player getVariable [QGVAR(exposure), MACRO_PLAYER_DEFAULTS_LOW]) < -10) && (call EFUNC(protection,totalProtection) select 0) < 1 && (call EFUNC(protection,totalProtection) select 1) < 1) then {
        player say3D [QEGVAR(audio,sound_sneeze),10,1,2,0];
    } else {
    if (((player getVariable [QGVAR(exposure), MACRO_PLAYER_DEFAULTS_LOW]) < -10) && (call EFUNC(protection,totalProtection) select 0) > 0 || (call EFUNC(protection,totalProtection) select 1) > 0) then {
