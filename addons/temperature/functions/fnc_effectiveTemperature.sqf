...
[_airTemp + _clothesWarmth, "temperature"] call EFUNC(common,addStatusModifier);

private _mainCondition = (_playerTemperature < 20) && (_parasites <= 0 || _infection <= 0);

// Early exit if main condition fails.
if (!_mainCondition) exitWith {};
private _exposureCondition = (abs _exposure) < 0.0001;

if ([player] call EFUNC(common,nearFire)) then {
    if (_exposureCondition) exitWith {
        player setVariable [QGVAR(exposure), MACRO_PLAYER_DEFAULTS_LOW];
    };

    private _nearFireCalculation = (((20 - _playerTemperature) * 2) / 5) / 5;

    [+_nearFireCalculation, "exposure"] call EFUNC(common,addStatusModifier);
    ...
    [+_sickNearFireCalculation, "exposure"] call EFUNC(common,addStatusModifier);
};
...
if (insideBuilding player isEqualTo 1) then {
    ...
    [+_insideWarmCalculation, "exposure"] call EFUNC(common,addStatusModifier);
    ...
    [+_sickInsideWarmCalculation, "exposure"] call EFUNC(common,addStatusModifier);
};
...
if !(isNull objectParent player) then {
    ...
    [+_insideVehicleCalculation, "exposure"] call EFUNC(common,addStatusModifier);
    ...
    [+_sickInsideVehicleCalculation, "exposure"] call EFUNC(common,addStatusModifier);
};
