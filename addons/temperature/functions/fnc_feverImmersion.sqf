...
[{!(isNil {player getVariable [QEGVAR(survival,exposure), MACRO_PLAYER_DEFAULTS_LOW]}) && ((player getVariable [QEGVAR(survival,exposure), MACRO_PLAYER_DEFAULTS_LOW]) > 10)},
{
    ...
    if (((player getVariable [QEGVAR(survival,exposure), MACRO_PLAYER_DEFAULTS_LOW]) > 10) || (_ailments find "PARASITES" isNotEqualTo -1 || _ailments find "INFECTION" isNotEqualTo -1) && (call EFUNC(protection,totalProtection) select 0) < 1 && (call EFUNC(protection,totalProtection) select 1) < 1) then {
    ...
    _ailments = player getVariable QCLASS(ailments);
        ...
        if (((player getVariable [QEGVAR(survival,exposure), MACRO_PLAYER_DEFAULTS_LOW]) > 10) || (_ailments find "PARASITES" isNotEqualTo -1 || _ailments find "INFECTION" isNotEqualTo -1) && (call EFUNC(protection,totalProtection) select 0) > 0 || (call EFUNC(protection,totalProtection) select 1) > 0) then {
