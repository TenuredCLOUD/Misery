...
_pricerads = parseNumber (((((player getVariable [QCLASS(radiation),0]) * 0.25) / 50) * Mis_Medpriceradheal) toFixed 1);
...
_priceradscalc = parseNumber (((player getVariable [QCLASS(radiation),0]) * 0.25) toFixed 1);
