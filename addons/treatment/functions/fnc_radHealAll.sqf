...
_pricerads = parseNumber ((((player getVariable [QEGVAR(survival,radiation),0]) / 50) * Mis_Medpriceradheal) toFixed 1);
...
_priceradscalc = parseNumber ((player getVariable [QEGVAR(survival,radiation),0]) toFixed 1);
